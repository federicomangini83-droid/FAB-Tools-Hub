...
# Bumped whenever the behaviour changes. Printed at startup so you can always
# tell which copy of the file is actually running.
__version__ = "2.1"
...
def write_zip(records: list[Record], folder: str, basename: str) -> str | None:
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        nested = [i.filename for i in entries if "/" in i.filename or "\\" in i.filename]
        log.info("Archive contents (%s entries):", len(entries))
        for info in entries:
            log.info("    %-45s %8s B", info.filename, info.file_size)
        if nested:
            log.error("NESTED ENTRIES FOUND: %s", nested)
        else:
            log.info("Archive is flat: no internal folder.")
...
def log_banner() -> None:
    ...
    If the values below do not match the file you are editing, you are running
    a different copy: fix that before debugging anything else.
    """
    log.info("=" * 68)
    log.info("Document Indexer v%s", __version__)
    log.info("Running file : %s", os.path.abspath(__file__))
    log.info("Modified     : %s", datetime.fromtimestamp(
        os.path.getmtime(os.path.abspath(__file__))
    ).strftime("%Y-%m-%d %H:%M:%S"))
    log.info("Python       : %s", sys.version.split()[0])
    log.info("Interpreter  : %s", sys.executable)
    log.info("Input folder : %s", DOCUMENT_FOLDER)
    log.info("Output folder: %s", OUTPUT_FOLDER)
    log.info(
        "Features     : zip=%s per-doc-json=%s page-markers=%s(every %s) table=%s",
        WRITE_ZIP,
        ZIP_PER_DOCUMENT_JSON,
        EMIT_PAGE_MARKERS,
        PAGE_MARKER_EVERY_WORDS,
        TABLE_FORMAT,
    )
    log.info("=" * 68)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract text, tables and images from DOCX/PDF for RAG."
    )
    parser.add_argument("-i", "--input", help="input folder (overrides DOCUMENT_FOLDER)")
    parser.add_argument("-o", "--output", help="output folder (overrides OUTPUT_FOLDER)")
    parser.add_argument("-n", "--name", help="output base name (overrides OUTPUT_BASENAME)")
    parser.add_argument("--no-ocr", action="store_true", help="disable OCR entirely")
    parser.add_argument("--no-zip", action="store_true", help="skip the zip archive")
    parser.add_argument("--clean", action="store_true",
                        help="delete previous outputs with the same base name first")
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    return parser.parse_args(argv)


def clean_previous_outputs(folder: str, basename: str) -> None:
    """Remove stale outputs so you never inspect a file from an older run."""
    if not os.path.isdir(folder):
        return
    suffixes = (".csv", ".json", ".txt", ".xlsx", ".zip", "_llm.txt")
    for filename in os.listdir(folder):
        ...
        if filename.endswith(suffixes):
            try:
                ...
                log.info("Removed previous output: %s", filename)
            except OSError as exc:
                log.warning("Could not remove %s: %s", filename, exc)


def main(argv: list[str] | None = None) -> None:
    global DOCUMENT_FOLDER, OUTPUT_FOLDER, OUTPUT_BASENAME, OCR_ENABLED, WRITE_ZIP

    args = parse_args(argv)
    if args.input:
        DOCUMENT_FOLDER = args.input
    if args.output:
    ...
    if args.clean:
        clean_previous_outputs(OUTPUT_FOLDER, OUTPUT_BASENAME)
    ...
    for path in write_outputs(records, OUTPUT_FOLDER, OUTPUT_BASENAME):
        ...
        log.info("Written: %s (%s B)", path, size)
